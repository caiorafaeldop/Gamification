...
const achievement_service_1 = require("../dist/services/achievement.service"); 
(0, dotenv_1.config)(); 

const prisma = new client_1.PrismaClient();
const HASH_ROUNDS = 10;

// ADAPTATION: Define constants that map to schema values
...
const TaskStatus = {
    ...
    REVIEW: "in_progress", // Mapped from REVIEW (missing) to in_progress
    BACKLOG: "todo",       // Mapped from BACKLOG (missing) to todo
    TODO: "todo"
};

async function main() {
    console.log("Start seeding (Adapted)...");
    ...
    for (let i = 1; i <= 20; i++) {
        const member = await prisma.user.upsert({
            ...
            create: {
                ...
                connectaPoints: Math.floor(Math.random() * 500),
                tierId: noviceTier.id,
    ...
    // 3. Create Teams (Mapped to Projects)
    // ADAPTATION: Using prisma.project instead of prisma.team
    // ADAPTATION: Mapping 'name' to 'title', merging 'focusArea' into 'description'
    const teamBackend = await prisma.project.upsert({
        where: { title: "Backend Development" },
        update: {},
        create: {
            title: "Backend Development",
            description: "Focus on API, database, and server logic. Focus Area: Node.js, PostgreSQL, Prisma",
            // focusArea removed
            leaderId: leader1.id,
            members: {
                create: [
    ...
    const teamFrontend = await prisma.project.upsert({
        where: { title: "Frontend Development" },
        update: {},
        create: {
            title: "Frontend Development",
            description: "Building user interfaces and user experience. Focus Area: React, TypeScript, Tailwind CSS",
            leaderId: leader2.id,
            members: {
                create: [
                    { userId: leader2.id },
    ...
    const teamResearch = await prisma.project.upsert({
        where: { title: "Research & Innovation" },
        update: {},
        create: {
            title: "Research & Innovation",
            description: "Exploring new technologies and solutions. Focus Area: AI, Machine Learning, Data Science",
            leaderId: leader3.id,
            members: {
                create: [
                    { userId: leader3.id },
    ...
    console.log("Projects (Teams) created:", teamBackend.title, teamFrontend.title, teamResearch.title);
    ...
    const tasks = [
        {
            ...
            pointsReward: 40,
            projectId: teamBackend.id, // ADAPTATION: teamId -> projectId
    ...
    for (const taskData of tasks) {
        await prisma.task.upsert({
            where: { id: taskData.title.replace(/\s/g, "-").toLowerCase() }, 
            update: {},
            create: taskData,
        });
    }
    ...
    const achievementsData = [
        ...
        { name: "Team Player", description: "Join a team.", icon: "🤝", criteria: "joined team" },
    ];
    ...
    console.log("Achievements created:", achievements.map((a) => a.name).join(", "));

    // Recalculate tiers
    const allUsers = await prisma.user.findMany({ include: { tier: true } });
    for (const user of allUsers) {
        // ... (rest of logic same as original but using safe imports)
