...
const GroupCard: React.FC<GroupCardProps> = ({ group, isMember, preview = false }) => {
  ...
  return (
    <article
      ...
      className={`group relative flex h-full flex-col rounded-2xl border border-gray-100 bg-surface-light shadow-sm transition-all duration-300 dark:border-gray-800 dark:bg-surface-dark ${preview ? 'cursor-default' : 'cursor-pointer hover:shadow-2xl'}`}
