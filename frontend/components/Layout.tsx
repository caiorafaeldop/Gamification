...
const Layout = () => {
    ...
    const signOut = () => clearSession({ reload: true });
        ...
        <div className="flex h-screen bg-background-light dark:bg-background-dark text-slate-800 dark:text-gray-100 font-sans transition-colors duration-300 overflow-hidden">
            ...
            <aside className="hidden md:flex w-64 bg-surface-light dark:bg-surface-dark border-r border-gray-200 dark:border-gray-800 flex-col sticky top-0 h-screen">
                <div className="h-16 flex items-center px-6 border-b border-gray-100 dark:border-gray-800/50">
                    <div className="flex items-center gap-2 cursor-pointer p-2 rounded-md bg-gray-50/50 dark:bg-white/5 border border-gray-100 dark:border-white/5 shadow-sm shadow-primary/5 hover:shadow-md hover:shadow-primary/10 transition-all duration-300" onClick={() => navigate('/')}>
                ...
                <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-1 custom-scrollbar">
                    ...
                    {!isGuest && <SidebarItem to="/dashboard" icon={LayoutDashboard} label="Dashboard" />}
                ...
                <div className="p-4 border-t border-gray-100 dark:border-gray-800">
                    {isGuest ? (
                        <button
                            ...
                            className="w-full flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-primary to-sky-500 shadow-lg shadow-primary/25 hover:from-sky-500 hover:to-primary transition-all"
                        >
                            <LogIn size={16} />
                            Entrar / Cadastrar
                        </button>
                ...
                <header className="h-16 flex items-center justify-between px-4 sm:px-6 bg-surface-light/80 dark:bg-surface-dark/80 backdrop-blur-md border-b border-gray-200 dark:border-gray-800 sticky top-0 z-30">
                    ...
                    <div className="flex items-center gap-3">
                        <a
                            ...
                            className="hidden sm:flex shrink-0 items-center gap-2 rounded-full bg-[#5865F2] px-4 py-2 text-[11px] font-bold text-white shadow-lg shadow-[#5865F2]/20 transition-all hover:scale-105 hover:bg-[#4752C4] active:scale-95 group"
                        >
                        ...
                        {isGuest ? (
                            <button
                                onClick={() => navigate('/login')}
                                className="flex items-center gap-2 rounded-full bg-gradient-to-r from-primary to-sky-500 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-primary/25 transition-all hover:scale-105 active:scale-95"
                            >
                                <LogIn size={14} />
                                Entrar / Cadastrar
                            </button>
