...
const TaskModal = ({ isOpen, onClose, onSuccess, task, projectId: defaultProjectId, initialColumnId, projectMembers }: TaskModalProps) => {
  ...
  if (isMobile) {
    const mobileProps = {
        navigate: (to: any) => { if (to === -1) onClose(); },
        location: { state: { projectId: defaultProjectId } }, // Mock location for project check
        user: currentUser,
        projects,
        loadingProjects,
        users,
        loadingUsers,
        taskLevel,
        points: taskLevel === 'basic' ? 50 : taskLevel === 'medium' ? 100 : 200, // Derived
        title,
        description,
        projectId,
        assignedToId,
    ...
    return (
        <div className="fixed inset-0 z-50 bg-background-dark">
             <MobileNewTaskScreen {...mobileProps} />
        </div>
    );
  }
